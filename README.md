...
## 구현
  ...
  - 로그인하면 이전 요청으로 돌아가는 로직 처리
  - 댓글 생성, 수정, 삭제
  - [댓글조회](#댓글조회) 요청 시 댓글이 속한 질문으로 리다이렉트
## 5단계 회고
- 나름 목적에 따른 계층 분리를 적용했다고 생각했다. 하지만 커스텀 예외를 생성하여 적용하면서 컨트롤러를 보니 굉장히 많은 서비스 로직이 있었다. 컨트롤러가 뚱뚱하다고 `session` 로직과 `try-catch` 때문인줄 알았는데 다시보니 그냥 잘 못 짠거였다.
## 피드백
...
## 고민
커스텀 예외를 생성할 것이였다면 조금 더 구체적인 예외를 생성했어야 하는 의문이 든다. 지금 생성한 것은 기존에 있는 것으로도 충분해보인다.
구체적이라면 예외 메시지 별로 예외를 만들어야할까?
## 기타
...
- #### 댓글조회
댓글 조회라는 말이 좀 이상하다. 게시글 상세 조회를 하면 보이는 것이 댓글인데, 댓글만 따로 본다? 뭔가 이상하다.
또 요청을 아래와 같이 보내보았더니 해당 요청에 대한 처리가 없다고 오류가 발생하였다.
```
localhost:8080/articles/1/replies/1
```
![img_22.png](img_22.png)

- 해결  
해당 요청에 대한 `GET` 처리를 만들어주었다. 하지만 댓글 조회가 아닌 해당 댓글이 속한 아티클 조회로 처리하였다.
또 url 해시를 통해 해당 댓글로 스크롤되게끔 구현하였다.   
